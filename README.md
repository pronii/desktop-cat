...
- 好友同屏：通过 WebSocket 房间同步宠物状态，输入 6 位房间码即可看到同房间里的其他小猫，并优先展示好友的 Live2D 形象。
...
```text
dist/desktop-cat-0.3.16-win-x64-setup.exe
dist/desktop-cat-0.3.16-win-x64-portable.exe
```
...
房间服务端可以提供浏览器管理页，用来查看当前在线客户端数、客户端 IP、设备标识和授权状态。客户端启动并联网后会定时向服务端上报心跳，后台按设备标识去重统计正在使用软件的客户端，不以是否加入房间为准。管理 API 需要 `DESKTOP_CAT_ADMIN_TOKEN`，浏览器管理页使用 `DESKTOP_CAT_ADMIN_PASSWORD` 登录并保存 HttpOnly 会话：
...
{
  "version": "0.3.16",
  "url": "https://example.com/releases/desktop-cat-0.3.16.exe",
  "sha256": "64位十六进制sha256",
  "notes": "客户端联网后主动上报在线心跳，后台按设备统计正在使用软件的客户端。",
  "mandatory": false
}
```
